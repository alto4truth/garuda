...
static cl::opt<bool> GenerateJSON("json", cl::desc("Generate JSON output"));
...
protected:
    std::shared_ptr<AnalysisResult> analyzeFunction(llvm::Function* function) override {
        ...
        auto group = std::make_shared<GroupDomain>(function->getName());
        group->setTop();
        result->setGroupDomain(function->getName(), group);
